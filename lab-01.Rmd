...
visitors %>% count()
```
# nrow(visitors) to calculate the number of rows
# ncol(visitors) to calculate the number of columns
...
All values in the output are `NA`! (unless na.rm parameter is used)

### Exercise i.

_What does `NA` stand for and why are you getting this as your answer to the previous question._

NA appears where an empty field is present (data is missing from the table). The NA seems to appear as a result because there are empty fields in the n_2021 column, meaning that the mean and median calculations couldn't be performed.
...
The na.rm parameter ensures that the NA values in the table are being ignored, so that an accurate calculaion can still take place. 
...
This suggests that the data on the number of visitors in 2022 across all attractions is positively skewed. That means that there are a few attractions that have a very high number of visitors, which affects the overall mean, but not the median (middle) value.
...
visitors %>%
  ...
  summarise(
    total = n(),
    free = sum(admission == "Free", na.rm = TRUE),
    proportion_free = free / total
  )
```
...
visitors %>%
  mutate(
    p_change = ((n_2022 - n_2021) / n_2021) * 100
  ) %>%
  filter(region == "Scotland") %>%
  group_by(admission) %>%
  summarise(
    mean_percentage_change = mean(p_change, na.rm = TRUE)
  ) %>%
